...
%   uses a single discount rate of 3 percent
%   uses a fixed climate sensitivity of 3 (the median value) (no Monte
%       Carlo simulation)
%   uses only the first of the five economic scenarios: "IMAGE" 
%   applies a share of damages to TFP
%==========================================================================

%clear all; close all; clc;
%set(0,'DefaultAxesFontName','Calibri','DefaultAxesFontSize',18);
%figsize = [500 200 800 750];
%rand('seed',4000);
...
% Input parameters:
rate = 0.03;      % Fixed discount rate
a    = 0.0028388; % Coefficient of damage function
b    = 2;         % Exponent of damage function
CS   = 3;         % Climate sensitivity [deg C]
H    = 2300;      % Time horizon for calculating the SCC [year]
eta  = 2;         % Will only be used if Ramsey discounting is turned on
rho  = 0.01;      % Will only be used if Ramsey discounting is turned on

%turn off Monte Carlo simulation
%MC = 1000;
%iterations = 5*3*MC; count = 0; starttime = cputime;

%for scenario = 1:5;
scenario = 1; %use only IMAGE scenario
...
warning('on','all');
y = Y./(N+eps);
% Import climate sensitivity pdf:
InputFile = [pwd '/2009 11 23 Calibrated R&B distribution.xls'];
p = xlsread(InputFile,'Sheet1'); 
CSdomain = p(:,1);
pCS      = p(:,2);
%--------------------------------------------------------------------------
...
%     count = count + 1;
%     if fix(mc/100)-mc/100==0;
%         clc;
%         fprintf(['Working on ' scenarioname ', rate = ' num2str(rate) '; ']);
%         fprintf(' %-.0f of %-.0f minor iterations.\n',mc,MC); 
%         fprintf('    [Est time to completion = %-.1f min]\n',...
%             (cputime-starttime)/count*(iterations-count)/60);
%     end;
    %--------------------------------------------------------------------------
    
    % Take random draw of CS from pdf:
    %temp = find(cumsum(pCS)<=rand);
    %CS = CSdomain(temp(end)); %turn this off--CS is set to 3, above
    ...
    if phi==0; % if no damages to TFP
        ...
        for tau = years(t):H-1; tt = tt + 1;
            SCC(t) = SCC(t) + ( C0i(tt) - C1i(tt) ) * 1/((1+rate)^(tt-1)); 
        end;
        end;
    ...
    SCC = SCC / (10^-3) * 12/44; 
    %SCC_MC(mc,:) = SCC'; building Monte Carlo summary - turned off
     
%end; Monte Carlo run - turned off

%turn off data export
%sheet = [scenarioname '_' num2str(rate)];
%xlswrite([pwd ['\SCC_DICE2007_EPA_output_' date '.xls']],[years';SCC_MC],sheet);
